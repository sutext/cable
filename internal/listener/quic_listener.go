...
func NewQUIC(logger *xlog.Logger, queueCapacity int32, config *quic.Config) Listener {
	if config == nil {
...
func (l *quicListener) OnClose(handler func(c Conn)) {
	l.closeHandler = handler
}
func (l *quicListener) OnAccept(handler func(p *packet.Connect, c Conn) packet.ConnectCode) {
	l.acceptHandler = handler
}
func (l *quicListener) OnPacket(handler func(p packet.Packet, c Conn)) {
...
func newQUICConn(id *packet.Identity, raw *quic.Stream, logger *xlog.Logger, queueCapacity int32) Conn {
	t := &quicConn{
		id:  id,
		raw: raw,
	}
